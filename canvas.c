...
static int px_set_counter = 0;
static double flood_start = 0;
...
static void canvas_window_setup() {
	...
	if (canvas_display >= 0) {
		...
		canvas_win = glfwCreateWindow(1024, 1024, "Pixelflut", NULL, NULL);
	}
...
static void* canvas_render_loop(void * arg) {
	...
	while ("pixels are coming") {
		...
		double now = glfwGetTime();
                ...
                /*
		if(px_set_counter > 0 && flood_start == 0.) {
			flood_start = now;
		} else if (px_set_counter > 0 && px_set_counter < 512*511) {
			// if we are currently flooding the canvas, print the
			// pixel throughtput
		        fprintf(stderr, "%.6f\n", (double) px_set_counter / (now - flood_start));
		}
		*/
